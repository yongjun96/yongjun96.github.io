...
1. 작업 진행 과정
 - 회원이 상담 신청 -> 상담 배당 -> 상담에 필요한 마이데이터 확인 -> 데이터를 저장하고 관리 -> PDF 및 엑셀로 정보 다운로드
...
### 서버에서 스크랩핑 API 호출
 ...
 - 여기서 고려되어야 하는 사항
   ...
   - 상담이 실패하는 경우, 스크랩핑이 성공해도  마이데이터 저장은 진행하지 않는다.
   - 상담은 생성되었지만 스크랩핑이 실페해서 마이데이터 저장이 되지 않았다면 스크랩핑을 다시 돌릴 수 있는 로직이 필요 
<br><br>
