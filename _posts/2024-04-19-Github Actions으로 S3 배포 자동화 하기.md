...
tags: [BackEnd, Github Action, S3, 환경변수, CI/CD]
---
...
- `Github Actions`란?
  ...
  - 여기서는 `CI/CD`를 수행 하기 위해 작업을 진행 했고, 배포 이외에도 여러 작업을 수행할 수 있다.
...
위 코드를 보면 `{% raw %}{{ }}{% endraw %}` 감싸진 변수들을 볼 수 있는데, `GitHub Actions`에서 민감 정보를 관리할 수 있는 현경 변수를 세팅한 것이다.
...
`S3`에 간단하게 배포하는 `CI/CD`를 작성해 보았다.  
