...
`jenkins`는 서버를 따로 만들어 관리해 줘야 한다는 불편함이 있었는데 `GitHub Actions`은 그런 불편함을  
해소해 주었다.  
하지만 `private`프로젝트는 일정 시간 이후에는 과금이 된다는 점이 부담이 될 수도 있다는 생각이 많이 들었다.
<br>
`CI/CD` 테스트를 진행할 때 `fork`를 따로 만들어서 진행해야 한다는 사실도 알게 되었다.
